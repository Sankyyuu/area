...
class Rappel extends React.Component {
	...
	componentDidMount() {
		this.props.updateData({name: "rappel", timeZone: this.state.timeZone, comparedTime: this.state.timeToSend})
	}
	...
	handlecomparedTime = async (e, data) => {
	...
	render() {
		...
		<div>
			...
			<div>
				<Select placeholder='Select the alert time' value={this.state.comparedTime} options={comparedTimeOptions} onChange={this.handlecomparedTime} style={{width: "95%", marginTop:"12px"}}/>
			</div>
		</div>
		)
	}
